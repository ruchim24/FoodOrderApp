...
import RestrautantCard, { withPromotedLabel } from "./RestraurantCard";
...
const Body = () => {
  ...
  const RestraurantCardPromoted = withPromotedLabel(RestrautantCard);
  ...
  const handleClick = () => {
    const filteredData = listOfRestraurants?.filter(
      (y) => y?.info?.avgRating > 4
    );
    setData(filteredData);
  };

  const handleSearch = () => {
    const filteredData = listOfRestraurants?.filter((res) =>
    ...
    <div className="body">
      <div className="filter flex">
        <div className="search mx-8 my-2 p-2">
          ...
          />
          <button
            className="h-14 px-4 bg-orange-300 ml-2 rounded-lg"
        ...
        <button
          className="h-14 p-4 bg-orange-300 m-4 rounded-lg"
          onClick={() => handleClick()}
        >
        ...
        {data?.map((res) => {
          return (
            <Link key={res.info.id} to={"/restaurant/" + res.info.id}>
              {res?.info?.veg ? (
                <RestraurantCardPromoted resData={res.info} />
              ) : (
                <RestrautantCard resData={res.info} />
              )}
            </Link>
          );
        })}
